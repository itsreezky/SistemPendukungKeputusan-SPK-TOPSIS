...
<div class="users-table table-wrapper mt-5">
    <center><b>SKALA NILAI HARGA</b> <br><br>
        ...
        <button class="btn btn-success" id="save-harga" style="display: none;">Simpan</button>
        <button class="btn btn-danger" id="delete-harga" style="display: none;">Hapus</button>
    </center>
                ...
                <?php foreach ($alternatif as $h2) : ?>
                    <?php if ($h1['id'] < $h2['id']) : ?>
                        <tr>
                            ...
                            <td class="geomean-harga p-5">0</td>
                        </tr>
                    <?php endif; ?>
                <?php endforeach; ?>
            <?php endforeach; ?>
...
$(document).ready(function() {
    ...
    function updateGeomeans(geomeans) {
        $('tbody tr').each(function(index) {
            $(this).find('.geomean-harga').text(geomeans[index].toFixed(2));
        });
    }
    ...
    $('#submit-harga').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                showSuccessMessage('Geomean Harga telah dihitung.');
                $('#save-harga').show();
            },
    ...
    $('#save-harga').click(function() {
        ...
        $.ajax({
            url: '<?= base_url('function/skala_nilaiHarga/simpanDataHarga') ?>',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({ data: data }),
            success: function(response) {
    ...
    $('#delete-harga').click(function() {
        $.ajax({
            url: '<?= base_url('function/skala_nilaiHarga/hapusDataHarga') ?>',
            type: 'POST',
            success: function(response) {
                showSuccessMessage('Data harga telah dihapus.');
                enableCheckboxes();
                $('#save-harga').hide();
                $('#delete-harga').hide();
                $('tbody tr').find('.geomean-harga').text('0');
            },
            error: handleAjaxError
        });
    });
